...
--각 부서별 COMM 을 받는 사원 수 (0도 받는 것으로 처리)
SELECT DEPTNO, COUNT(COMM)
FROM EMP
GROUP BY DEPTNO
;

--각 부서별 COMM 을 받는 사원 수 (0 포함 안 하기)
...
--각 부서 별 평균급여 조회 (단, 평균급여가 2000 이상인 경우만)
...
--EMP 테이블에서 급여가 1000 이상인 사람들에 대하여, 부서 별 급여 합계를 조회
...
--각 부서 별, 보너스(COMM)를 받는 사원의 수 조회
SELECT DEPTNO 부서번호, COUNT(NULLIF(COMM,0)) 보너스
FROM EMP
GROUP BY DEPTNO
;
