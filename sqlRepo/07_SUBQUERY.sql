...
/*
    <서브 쿼리 구분>
        ...
        2) 다중행 서브 쿼리           : 서브 쿼리의 조회결과 값의 행의 개수가 여러행 일 때
...
/*
    <단일행 서브 쿼리>
        서브 쿼리의 조회 결과 값의 행과 열의 갯수가 1개일 때
        비교 연산자 사용 가능
*/
...
/*
    <다중행 서브 쿼리>
        서브 쿼리의 조회 결과 값의 행의 갯수가 여러 행일 때
        
        IN / NOT IN (서브 쿼리) : 여러 개의 결과값 중에서 한 개라도 일치한다면 TRUE 리턴
        ANY : 여러 개의 값들 중 한 개라도 만족하면 TRUE, IN 과 다른 점은 비교 연산자를 함께 사용한다는 점
            ANY(100, 200, 300)
            SALARY = ANY(...) : IN 과 같은 결과
            SALARY != ANY(...) : NOT IN 과 같은 결과
            SALARY > ANY(...) : 최소값 보다 크면 TRUE
            SALARY < ANY(...) : 최대값 보다 작으면 TRUE
        ALL : 여러 개의 값들 모두와 비교하여 만족해야 TRUE, IN 과 다른 점은 비교 연산자를 함께 사용한다는 점
            ALL(100,200,300)
...
FROM EMPLOYEE
--WHERE JOB_CODE = 'J1' AND SALARY = 해당 직급 최소 급여
--OR JOB_CODE = 'J2' AND SALARY = 해당 직급 최소 급여
--OR JOB_CODE = 'J3' AND SALARY = 해당 직급 최소 급여
--WHERE (JOB_CODE, SALARY) IN (('J1', 8000000)) , ('J2', 3700000) , (), (), .....)
...
/*
    <인라인 뷰>
        FROM 절에 서브 쿼리를 제시하고, 서브 쿼리를 수행한 결과를 테이블 대신 사용
*/
...
/*
    <RANK 함수>
        ...
        - RANK 와 DENSE_RANK 는 중복 순위 이후 값 처리가 다름
...
FROM EMPLOYEE
--WHERE DENSE_RANK() OVER(ORDER BY SALARY DESC) <= 5 --WHERE 절은 사용 불가능
;

--부서코드가 D9, D5 에 해당하는 사원 조회
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = ANY('D9', 'D5');

--SELECT *
--FROM EMPLOYEE
--WHERE SALARY > ALL(3000000, 5000000);
