...
-- 1) 칼럼 추가/수정/삭제
-- 1-1) 칼럼 추가(ADD) : ALTER TALBE 테이블명 ADD 칼럼명 테이터타입
--  1-2) 칼럼 수정(MODIFY)
...
-- 3) 테이블명/칼럼명/제약조건명 변경
--  3-1) 칼럼명 변경 : ALTER TABLE 테이블명 RENAME COLUMN 기존 칼럼명 TO 변경할 칼럼명;
--  3-2) 제약조건명 변경 : ALTER TABLE 테이블명 RENAME CONSTRAINT 기존 제약조건명 TO 변경할 제약조건명;
--  3-3) 테이블명 변경
