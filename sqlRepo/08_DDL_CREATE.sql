...
INSERT INTO TB_MEMBER(ID, PWD, NAME) VALUES(NULL, '1234', 'SWY');
...
CREATE TABLE TB_MEMBER (
    ...
    , NAME VARCHAR2(20) 
    , ENROLL_DATE TIMESTAMP DEFAULT SYSDATE
    , CONSTRAINT MEMBER_NO_ID_PWD_NAME_UQ UNIQUE(ID, PWD, NAME)
);
...
/*
    <기본 키>
        ...
        한 테이블에 한 개만 설정할 수 있다. (여러 개의 칼럼을 묶어서 기본 키로 지정 가능)
        칼럼 레벨, 테이블 레벨 설정 모두 가능
*/
...
CREATE TABLE TB_MEMBER (
    ...
    , PWD VARCHAR2(100)
    , NICK VARCHAR2(100)
    , CONSTRAINT MEMBER_PK_ID_PWD PRIMARY KEY(ID, PWD) --복합 키
);
...
/*
    <외래키>
        ...
        [문법]
            1) 칼럼 레벨
                 칼럼명 자료형(크기) [CONSTRAINT 제약조건명] REFERENCES 참조테이블명 [(기본키)] [삭제롤]
            ...
            [삭제롤]
                 ...
                 1) ON DELETE RESTRICT  : 
                     ...
                     부모 테이블의 행을 삭제할 수 없다. (기본 옵션)
                 2) ON DELETE SET NULL  : 
                     부모 테이블의 데이터 삭제 시, 참조하고 있는 자식 테이블의 칼럼 값이 NULL 로 변경
                 3) ON DELETE CASCADE   : 
                     부모 테이블의 데이터 삭제 시, 참조하고 있는 자식 테이블의 칼럼 값이 존재하는 행 전체 삭제
*/
